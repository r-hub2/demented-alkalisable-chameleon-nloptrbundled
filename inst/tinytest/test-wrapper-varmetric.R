...
varControl <- nloptr(
  ...
  lb = lb,
  ub = ub,
  opts = list(algorithm = "NLOPT_LD_VAR1", xtol_rel = 1e-8, maxeval = 1000L)
)
...
varControl <- nloptr(
  ...
  lb = lb,
  ub = ub,
  opts = list(algorithm = "NLOPT_LD_VAR2", xtol_rel = 1e-8, maxeval = 1000L)
)
...
# Passed gradient
fr <- function(x) {
  100 * (x[2L] - x[1L]^2)^2 + (1 - x[1L])^2
}
gr <- function(x) {
  .expr2 <- x[2L] - x[1L]^2
  .expr5 <- 1 - x[1L]
  c(-(2 * .expr5 + 100 * (2 * (2 * x[1L] * .expr2))), 100 * (2 * .expr2))
}
