...
expect_error(
  nloptr(x0 = c(-1.2, 1.5, NA), eval_f = fphv, opts = ctlNM),
  "x0 contains NA",
  fixed = TRUE
)

expect_error(
  nloptr(x0 = c(-1.2, 1.5, 3), eval_f = fphv, lb = c(-2, -2), opts = ctlNM),
  "length(lb) != length(x0)",
  fixed = TRUE
)

expect_error(
  nloptr(x0 = c(-1.2, 1.5, 3), eval_f = fphv, ub = c(-2, -2), opts = ctlNM),
  "length(ub) != length(x0)",
  fixed = TRUE
)
...
expect_error(
  nloptr(x0 = c(-1.2, 1.5, 3), eval_f = fphv, lb = c(-1, -1, -1), opts = ctlNM),
  "at least one element in x0 < lb",
  fixed = TRUE
)

expect_error(
  nloptr(x0 = c(-1.2, 1.5, 3), eval_f = fphv, ub = c(2, 2, 2), opts = ctlNM),
  "at least one element in x0 > ub",
  fixed = TRUE
)
...
expect_error(
  nloptr(
    x0 = c(-1.2, 1.5, 3),
    eval_f = fphv,
    opts = list(algorithm = "NLOPT_LN_NEDERMEAD", xtol_rel = 1e-8)
  ),
  "Incorrect algorithm supplied. Use one of the",
  fixed = TRUE
)
...
expect_error(
  suppressWarnings(nloptr(x0 = -1, eval_f = f0, opts = ctlNM)),
...
expect_error(
  suppressWarnings(nloptr(x0 = -1, f0, g0, opts = ctlLB)),
  "objective in x0 returns NA",
  fixed = TRUE
)
...
expect_warning(
  nloptr(x0 = -1, f0, g0, opts = ctlNM),
  "a gradient was supplied for the objective function",
  fixed = TRUE
)
expect_error(
  nloptr(x0 = -1, f0, opts = ctlLB),
  "A gradient for the objective function is needed",
  fixed = TRUE
)
...
## eval_g_ineq test
expect_error(
  is.nloptr(list(eval_f = fphv, eval_g_ineq = "GoodBye")),
  "eval_g_ineq is not a function"
)

## eval_g_eq test
expect_error(
  is.nloptr(list(eval_f = fphv, eval_g_eq = 27)),
...
expect_error(
  ...
  "inequality constraints in x0 returns NA",
  fixed = TRUE
)
...
expect_error(
  ...
  "wrong number of elements in jacobian of inequality",
  fixed = TRUE
)
...
expect_error(
  ...
  "gradient for the inequality constraints is needed",
  fixed = TRUE
)
...
expect_error(
  ...
  "wrong number of elements in jacobian of equality",
  fixed = TRUE
)
...
expect_error(
  ...
  "gradient for the equality constraints is needed",
  fixed = TRUE
)
...
expect_error(
  ...
  "The vector tol_constraints_eq in the options list",
  fixed = TRUE
)
