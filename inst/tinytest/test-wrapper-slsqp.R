...
expect_stdout(
  slsqp(x0.hs100, fn = fn.hs100, nl.info = TRUE),
  "Call:",
  fixed = TRUE
)
...
slsqpControl <- nloptr(
  ...
  opts = list(algorithm = "NLOPT_LD_SLSQP", xtol_rel = 1e-6, maxeval = 1000L)
)
...
slsqpTest <- suppressWarnings(slsqp(
  x0.hs100,
  fn = fn.hs100,
  gr = gr.hs100,
  hin = hin2.hs100,
