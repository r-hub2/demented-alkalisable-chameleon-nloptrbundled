...
is.nloptr <- function(x) {
  ...
  if (any(x$x0 < x$lower_bounds)) {
    stop("at least one element in x0 < lb")
  }
  if (any(x$x0 > x$upper_bounds)) {
  ...
  if (is.list(f0)) {
    ...
    if (x$options$algorithm %in% list_algorithms_d) {
      stop(
        "A gradient for the objective function is needed by ",
        "algorithm ",
        x$options$algorithm,
    ...
    if (is.list(g0_ineq)) {
      ...
      if (length(g0_ineq$jacobian) != length(g0_ineq$constraints) * lx0) {
        stop(
          ...
          ", but should be ",
          length(g0_ineq$constraints),
          " x ",
          lx0,
          " = ",
          length(g0_ineq$constraints) * lx0,
          ")"
        )
      }
      ...
      if (x$options$algorithm %in% list_algorithms_n) {
        warning(
          "a gradient was supplied for the inequality ",
          "constraints, but algorithm ",
          x$options$algorithm,
          " does not use gradients."
        )
      }
      ...
      if (x$options$algorithm %in% list_algorithms_d) {
        stop(
          "A gradient for the inequality constraints is needed by ",
          "algorithm ",
          x$options$algorithm,
          " but was not supplied.\n"
        )
  ...
  if (!is.null(x$eval_g_eq)) {
    ...
    if (is.list(g0_eq)) {
      ...
      if (x$options$algorithm %in% list_algorithms_n) {
        warning(
          "a gradient was supplied for the equality ",
          "constraints, but algorithm ",
          x$options$algorithm,
          " does not use gradients."
        )
      ...
      if (x$options$algorithm %in% list_algorithms_d) {
        stop(
          "A gradient for the equality constraints is needed by ",
          "algorithm ",
          x$options$algorithm,
          " but was not supplied.\n"
        )
  ...
  if (x$num_constraints_eq > 0) {
    ...
    if (!(x$options$algorithm %in% eq_algorithms)) {
      stop(
        "If you want to use equality constraints, ",
  ...
  if (x$num_constraints_ineq != length(x$options$tol_constraints_ineq)) {
    stop(
      "The vector tol_constraints_ineq in the options list has size ",
  ...
  if (x$num_constraints_eq != length(x$options$tol_constraints_eq)) {
    stop(
      "The vector tol_constraints_eq in the options list has size ",
