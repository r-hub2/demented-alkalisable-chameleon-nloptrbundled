...
# check x0
expect_error(
  nloptr("3", function(x) x, opts = ctlNM),
  "x0 must be numeric",
  fixed = TRUE
)
expect_error(
  nloptr(numeric(), function(x) x, opts = ctlNM),
  "x0 must have length > 0",
  fixed = TRUE
)
...
expect_error(
  nloptr(3, fn, b = "X", c = "Y", d = "Q"),
  "passed to (...) in 'nloptr' but this is not required in the",
  fixed = TRUE
)

expect_warning(
  nloptr(3, fn, b = 3, c = 4, opts = ctlNM),
...
minus2mess <- "NLOPT_SUCCESS: Generic success return value."
...
expect_true(
  grepl(
    "Optimization stopped because stopval",
    nloptr(
      c(4, 4),
      fn,
      opts = list(algorithm = "NLOPT_LN_SBPLX", stopval = 20)
    )$message,
    fixed = TRUE
  )
)
