...
#               (Avraham Adler)
#   2025-04-22: Fix seed via ranseed and re-increase tolerance to machine
...
expect_stdout(
  stogo(x0, rbf, lower = lb, upper = ub, nl.info = TRUE),
  "Call:",
  fixed = TRUE
)
...
stogoControl <- nloptr(
  ...
  lb = lb,
  ub = ub,
  opts = list(algorithm = "NLOPT_GD_STOGO", xtol_rel = 1e-6, maxeval = 10000L)
)
...
isresTest <- suppressWarnings(isres(
  x0,
  rbf,
  lb,
  ub,
