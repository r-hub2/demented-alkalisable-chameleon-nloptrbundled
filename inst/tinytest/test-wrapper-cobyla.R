...
expect_stdout(
  cobyla(x0.hs100, fn.hs100, nl.info = TRUE, deprecatedBehavior = FALSE),
  "Call:",
  fixed = TRUE
)
...
cobylaControl <- nloptr(
  x0 = x0.hs100,
  eval_f = fn.hs100,
  eval_g_ineq = hin.hs100,
  opts = list(algorithm = "NLOPT_LN_COBYLA", xtol_rel = 1e-8, maxeval = 1000L)
)
...
cobylaTest <- suppressWarnings(cobyla(
  x0.hs100,
  fn.hs100,
  hin = hin2.hs100,
  control = ctl
...
bobyqaControl <- nloptr(
  x0 = c(0, 0),
  eval_f = rbf,
  lb = c(0, 0),
  ub = c(0.5, 0.5),
  opts = list(algorithm = "NLOPT_LN_BOBYQA", xtol_rel = 1e-6, maxeval = 1000L)
)
...
newuoaControl <- nloptr(
  x0 = c(1, 2),
  eval_f = rbf,
  opts = list(algorithm = "NLOPT_LN_NEWUOA", xtol_rel = 1e-6, maxeval = 1000L)
)
